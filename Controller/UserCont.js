...
const RegisterUser = async (req, res)=>{
    ...
    const {name, email, website, password, userId} = RgData;
    ...
    if(userData){
        ...
        const token = await jwt.sign({email: email}, process.env.SecreatKey, {expiresIn: "15d"})
...
const LoginUser = async (req, res)=>{
    ...
    if(userData){
        ...
        const token = jwt.sign({email: email}, process.env.SecreatKey, {expiresIn: '30d'})
